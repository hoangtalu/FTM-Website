...
function buildDemoItinerary(input) {
  ...
  return {
    ...
    itinerary: {
      ...
      visualBrief: buildVisualBrief(input, tour),
...
function buildVisualBrief(input, tour) {
  const regionLabel = regionMap[input.region];
  const destinationImages = tour.destinations.map((destination) => ({
    destination: destination.name,
    query: `${destination.name} Vietnam travel landscape`,
    alt: `${destination.name} in ${regionLabel}`
  }));

  return {
    heroImageQuery: `${regionLabel} Vietnam ${focusMap[input.tripFocus]} ${input.travelMonth} travel`,
    heroImageAlt: `${regionLabel} travel mood`,
    destinationImages,
    moodTags: [
      regionLabel,
      focusMap[input.tripFocus],
      comfortMap[input.comfortLevel],
      paceMap[input.paceStyle]
    ]
  };
}
