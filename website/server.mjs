...
const contextPath = path.join(__dirname, "data", "ftm-context.md");
  ...
  try {
    ...
    if ((request.method === "GET" || request.method === "HEAD") && url.pathname === "/api/config") {
      return sendJson(response, 200, {
        mode: geminiApiKey ? "grounded" : "demo",
        agency: agencyConfig,
        model: geminiModel
      }, request.method === "HEAD");
    }
    ...
    if (request.method === "POST" && url.pathname === "/api/itinerary") {
      ...
      const result = geminiApiKey
        ? await buildGroundedItinerary(input)
...
async function buildGroundedItinerary(input) {
